const races = [

    // =========================================================
    // 2026년 9월
    // =========================================================

    {
        name: "제14회 설악산공룡능선 CLIMBATHON",
        date: "2026-09-03",
        region: "강원",
        place: "설악산 한계령휴게소",
        distances: ["27km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 다이나핏 태백 트레일",
        date: "2026-09-05",
        region: "강원",
        place: "태백 소원지 오토캠핑장",
        distances: ["50K", "32K-T", "32K-U"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.taebaektrail.com/"
    },

    {
        name: "제12회 I LOVE 방송대 마라톤",
        date: "2026-09-05",
        region: "서울",
        place: "상암동 평화광장",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 Just Run10 세종",
        date: "2026-09-05",
        region: "세종",
        place: "세종마루공원 밑 금강변",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제23회 철원DMZ국제평화마라톤",
        date: "2026-09-05",
        region: "강원",
        place: "철원 고석정",
        distances: ["풀", "하프", "10km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://dmz.wizrun.com/"
    },

    {
        name: "제20회 순천만울트라마라톤",
        date: "2026-09-05",
        region: "전남",
        place: "순천동천천변공원",
        distances: ["100km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://scultra100.net/"
    },

    {
        name: "2026 Vrun 2회",
        date: "2026-09-05",
        region: "서울",
        place: "신정교하부육상트랙구장",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 이일선마라톤",
        date: "2026-09-05",
        region: "경북",
        place: "울릉도 나리분지",
        distances: ["10km", "4.4km 걷기"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제23회 희망드림 새벽강변 국제마라톤",
        date: "2026-09-06",
        region: "서울",
        place: "목동종합운동장",
        distances: ["하프", "10km", "5km", "3km 가족런"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://amarunsb.com/"
    },

    {
        name: "2026 KMA 청주마라톤",
        date: "2026-09-06",
        region: "충북",
        place: "청주 무심천 체육공원",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제24회 청광종주 트레일러닝",
        date: "2026-09-06",
        region: "서울",
        place: "양재동 화물터미널",
        distances: ["26km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제11회 김대중평화마라톤",
        date: "2026-09-06",
        region: "서울",
        place: "뚝섬 한강공원 수변무대",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 문경새재 트레일런",
        date: "2026-09-06",
        region: "경북",
        place: "문경새재도립공원 제1주차장",
        distances: ["13km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제9회 인천서구 단축마라톤",
        date: "2026-09-06",
        region: "인천",
        place: "청라호수공원 멀티프라자 트랙",
        distances: ["10km", "4.3km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 봉화송이마라톤",
        date: "2026-09-06",
        region: "경북",
        place: "봉화공설운동장",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 Run in 영덕",
        date: "2026-09-06",
        region: "경북",
        place: "영덕 대진해수욕장~고래불해수욕장",
        distances: ["8km", "4km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 비포 선라이즈 슬로우 마라톤",
        date: "2026-09-12",
        region: "강원",
        place: "강릉 경포호수",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "Bbang Trail Run 2026",
        date: "2026-09-12",
        region: "강원",
        place: "정선 하이원 리조트",
        distances: ["30K", "20K", "12K"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제2회 초록우산 런웨이 마라톤",
        date: "2026-09-12",
        region: "대전",
        place: "대전엑스포시민광장",
        distances: ["하프", "10km", "5km", "3km 걷기", "3km 어린이 걷기"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 장수 레드푸드레이스",
        date: "2026-09-12",
        region: "전북",
        place: "장수말역사체험관",
        distances: ["16km", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 굿네이버스 레이스",
        date: "2026-09-12",
        region: "서울",
        place: "여의도 한강공원 물빛무대",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://gnrace.kr/"
    },

    {
        name: "제6회 에그마라톤",
        date: "2026-09-12",
        region: "경기",
        place: "경인아라뱃길여객터미널 아라타워",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 88run",
        date: "2026-09-12",
        region: "경기",
        place: "미사경정공원",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://88run.kr/"
    },

    {
        name: "제2회 화이트런 기부마라톤",
        date: "2026-09-12",
        region: "서울",
        place: "영등포구 신정교하부 육상트랙구장",
        distances: ["10km", "4.2km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 에너지 히어로 레이스",
        date: "2026-09-12",
        region: "서울",
        place: "서울월드컵경기장 평화광장",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.herorace.kr/"
    },

    {
        name: "2026 여수 백리섬섬 트레일런",
        date: "2026-09-12",
        region: "전남",
        place: "청심 국제해양청소년수련원",
        distances: ["40K", "27K", "16K"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.speedtrailrun.kr/"
    },

    {
        name: "2026 양양강변 전국마라톤",
        date: "2026-09-12",
        region: "강원",
        place: "강원도 양양군 웰컴센터",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 나이트런 플러스 청주",
        date: "2026-09-12",
        region: "충북",
        place: "청주시 무심천체육공원",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 히어로레이스",
        date: "2026-09-13",
        region: "서울",
        place: "여의도공원 문화의마당",
        distances: ["15km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 러닝월드 10K",
        date: "2026-09-13",
        region: "대전",
        place: "대전엑스포시민광장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 포항 이차전지 전국마라톤대회",
        date: "2026-09-13",
        region: "경북",
        place: "포항운하관 광장",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제16회 스마일런 페스티벌",
        date: "2026-09-13",
        region: "서울",
        place: "상암동 평화의공원 평화광장",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.smilerun.co.kr/"
    },

    {
        name: "제26회 강화바다마라톤",
        date: "2026-09-13",
        region: "인천",
        place: "강화함상공원",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 울진 금강송 힐링마라톤",
        date: "2026-09-13",
        region: "경북",
        place: "울진종합운동장",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 런서울런",
        date: "2026-09-13",
        region: "서울",
        place: "서울광장",
        distances: ["하프", "10km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://runseoulrun.com/"
    },

    {
        name: "제19회 구로 안양천 가을마라톤",
        date: "2026-09-13",
        region: "서울",
        place: "구일역 하부 인라인스케이트장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 장수 트레일레이스 Fall",
        date: "2026-09-19",
        region: "전북",
        place: "장수종합경기장",
        distances: ["100km", "38K-J"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제18회 사이버영토수호 마라톤",
        date: "2026-09-19",
        region: "서울",
        place: "여의도 한강공원 물빛무대",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 Just Run10 김해 대동",
        date: "2026-09-19",
        region: "경남",
        place: "김해 대동생태체육공원 축구장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 안양천 하프마라톤",
        date: "2026-09-19",
        region: "서울",
        place: "신정교 하부 영롱이 억새구장",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 시티포레스티벌 런핏 디스커버리",
        date: "2026-09-19",
        region: "경기",
        place: "고양시 일산호수공원",
        distances: ["10km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 사찰런",
        date: "2026-09-19",
        region: "경북",
        place: "경산 갓바위 소원길",
        distances: ["6.3km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 남동청년런",
        date: "2026-09-19",
        region: "인천",
        place: "인천대공원",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제2회 경기신문 씨사이드마라톤",
        date: "2026-09-19",
        region: "인천",
        place: "영종구 씨사이드파크",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 금산인삼축제 마라톤",
        date: "2026-09-19",
        region: "충남",
        place: "금산세계인삼엑스포주차장",
        distances: ["하프", "10km", "4km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 여수 낭도 트레일",
        date: "2026-09-19",
        region: "전남",
        place: "여수 화정면 낭도출장소 앞 주차장",
        distances: ["13km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 무도런 경주",
        date: "2026-09-19",
        region: "경북",
        place: "경주시민운동장",
        distances: ["하프", "10km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제4회 문경오미자런",
        date: "2026-09-19",
        region: "경북",
        place: "문경종합온천 뒤 주차장",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 공주백제마라톤",
        date: "2026-09-20",
        region: "충남",
        place: "공주시민운동장",
        distances: ["풀", "32km", "하프", "10km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.gongjumarathon.com/"
    },

    {
        name: "2026 한돈런",
        date: "2026-09-20",
        region: "경기",
        place: "미사 조정경기장",
        distances: ["10km", "6km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://handonrun.com/"
    },

    {
        name: "2026 인천송도국제마라톤",
        date: "2026-09-20",
        region: "인천",
        place: "국립 인천대학교 송도캠퍼스 정문 앞",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://songdorun.net/"
    },

    {
        name: "2026 울산 남구청장배 마라톤",
        date: "2026-09-20",
        region: "울산",
        place: "태화강국가정원 왕버들 공연장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제19회 가평 자라섬마라톤",
        date: "2026-09-20",
        region: "경기",
        place: "가평종합운동장",
        distances: ["하프", "10km", "10km 커플런", "5km", "4.2km 패밀리런"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.gprun.com/"
    },

    {
        name: "2026 동대문마라톤",
        date: "2026-09-20",
        region: "서울",
        place: "중랑천 제1수변공원",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제2회 마포구청장배 마라톤",
        date: "2026-09-20",
        region: "서울",
        place: "평화의공원 평화광장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 제20회 선사마라톤",
        date: "2026-09-20",
        region: "서울",
        place: "서울 암사동 유적 앞 특설무대",
        distances: ["하프", "10km", "5km", "5km 걷기"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 안산마라톤",
        date: "2026-09-20",
        region: "경기",
        place: "대부향기테마파크",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    // =========================================================
    // 2026년 10월
    // =========================================================

    {
        name: "2026 Trans Jeju UTMB",
        date: "2026-10-02",
        region: "제주",
        place: "제주월드컵경기장",
        distances: ["100km", "70km", "20km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 완주트레일런",
        date: "2026-10-03",
        region: "전북",
        place: "완주군 고산자연휴양림",
        distances: ["36K", "22K", "7K"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 마스터즈 하프마라톤",
        date: "2026-10-03",
        region: "서울",
        place: "상암 월드컵공원 평화광장",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 Just Run10 청주 1224",
        date: "2026-10-03",
        region: "충북",
        place: "무심천체육공원",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제2회 아산 이순신 트레일",
        date: "2026-10-03",
        region: "충남",
        place: "아산 신정호 이순신장군 동상",
        distances: ["60km", "46km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 서산 코스모스 황금들판 마라톤",
        date: "2026-10-03",
        region: "충남",
        place: "서산스포츠테마파크",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제25회 김제 새만금 지평선 마라톤",
        date: "2026-10-03",
        region: "전북",
        place: "김제시민운동장",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 천사데이 동두천마라톤",
        date: "2026-10-03",
        region: "경기",
        place: "경기도 동두천시 캠프보산",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 춘천연합뉴스마라톤",
        date: "2026-10-03",
        region: "강원",
        place: "춘천시 엘리시안 강촌",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

   

    {
        name: "2026 ROK Army Trail Running",
        date: "2026-10-03",
        region: "경기",
        place: "용인시 처인구 육군 특전사 훈련코스",
        distances: ["16.25km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 S-OIL 50주년 마라톤",
        date: "2026-10-04",
        region: "서울",
        place: "월드컵공원 평화광장",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 YTN 서울투어마라톤",
        date: "2026-10-04",
        region: "서울",
        place: "서울광장",
        distances: ["하프", "10km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://run.ytn.co.kr/"
    },

    {
        name: "2026 런유어웨이 서울 10K",
        date: "2026-10-04",
        region: "서울",
        place: "여의도공원 문화의광장",
        distances: ["10km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제20회 달서 하프마라톤",
        date: "2026-10-04",
        region: "대구",
        place: "대구 달서구 호림강나루공원",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://dalseomarathon.com/"
    },

    {
        name: "제23회 경남마라톤",
        date: "2026-10-04",
        region: "경남",
        place: "창원종합운동장 보조경기장",
        distances: ["10km", "3km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 파주북시티마라톤",
        date: "2026-10-04",
        region: "경기",
        place: "파주출판도시",
        distances: ["10km", "5km", "3km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.bookcity.or.kr/"
    },

    {
        name: "2026 안동마라톤",
        date: "2026-10-04",
        region: "경북",
        place: "안동시민운동장",
        distances: ["풀", "하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://andongmarathon.com/"
    },

    {
        name: "2026 홍천사랑마라톤",
        date: "2026-10-04",
        region: "강원",
        place: "홍천종합운동장",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제14회 광명건강달리기",
        date: "2026-10-04",
        region: "경기",
        place: "광명 찬빛광장",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.gmrun.kr/"
    },

    {
        name: "2026 소노 런트립 경주",
        date: "2026-10-04",
        region: "경북",
        place: "소노캄 경주",
        distances: ["9km", "7km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.sonohotelsresorts.com/calm_gju"
    },

    {
        name: "2026 한경 서울마라톤",
        date: "2026-10-05",
        region: "서울",
        place: "여의도공원 문화의마당",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.emarathon.or.kr/"
    },

    {
        name: "Run Sense 2026",
        date: "2026-10-05",
        region: "서울",
        place: "서울 상암동 월드컵공원 평화광장",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 서울오픈마라톤",
        date: "2026-10-05",
        region: "서울",
        place: "광화문광장",
        distances: ["하프", "10km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://seoulopen.or.kr/"
    },

    {
        name: "제12회 시흥전국하프마라톤",
        date: "2026-10-05",
        region: "경기",
        place: "한국공학대학교 제2캠퍼스",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제23회 강남국제평화마라톤",
        date: "2026-10-05",
        region: "서울",
        place: "봉은사로 삼성1동 주민센터",
        distances: ["풀", "하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://peacemarathon.co.kr/"
    },

    {
        name: "2026 양주 천일홍 가을페스타 마라톤",
        date: "2026-10-05",
        region: "경기",
        place: "양주시 나리공원",
        distances: ["20km", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제27회 홍성마라톤",
        date: "2026-10-09",
        region: "충남",
        place: "홍주종합경기장",
        distances: ["하프", "10km", "10km 단체전", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

   
    {
        name: "2026 한글런",
        date: "2026-10-09",
        region: "충남",
        place: "세종시 중앙공원 도시축제마당",
        distances: ["10.9km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 생명사랑 마라톤/걷기대회",
        date: "2026-10-09",
        region: "경기",
        place: "미리내성지",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "빵빵런 2026 대전",
        date: "2026-10-09",
        region: "대전",
        place: "대전 엑스포 시민광장",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 고흥 우주항공 마라톤",
        date: "2026-10-09",
        region: "전남",
        place: "고흥군 박지성공설운동장",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제5회 무등산 그레이트 마라톤",
        date: "2026-10-10",
        region: "전남",
        place: "화순 금호스파리조트",
        distances: ["30km", "하프", "10km", "5.18km", "2km 걷기"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제4회 강서 허준런",
        date: "2026-10-10",
        region: "서울",
        place: "서울식물원 초지원",
        distances: ["10km", "5km", "3km", "패밀리런"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 카카오프렌즈런",
        date: "2026-10-10",
        region: "경기",
        place: "탄천 종합운동장",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제9회 거제섬꽃마라톤",
        date: "2026-10-10",
        region: "경남",
        place: "거제스포츠파크",
        distances: ["하프", "10km", "4km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 경포마라톤",
        date: "2026-10-10",
        region: "강원",
        place: "강원특별자치도 강릉시",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제5회 구미시장배 금오산 트레일",
        date: "2026-10-10",
        region: "경북",
        place: "금오산 공영 대주차장",
        distances: ["23km", "18km", "10km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제3회 성동구청장배 마라톤",
        date: "2026-10-10",
        region: "서울",
        place: "살곶이대운동장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 충주 앙성 비내길 마라톤",
        date: "2026-10-10",
        region: "충북",
        place: "충주 앙성온천광장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 제주 바람개비런",
        date: "2026-10-10",
        region: "제주",
        place: "한림수협 다목적어업인종합지원센터",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제3회 서울트레일런",
        date: "2026-10-10",
        region: "서울",
        place: "시청 덕수궁돌담길",
        distances: [],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 하트런",
        date: "2026-10-10",
        region: "서울",
        place: "올림픽공원 피크닉장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://heartrun.co.kr/"
    },

    {
        name: "제7회 천안삼거리 흥타령 울트라마라톤",
        date: "2026-10-10",
        region: "충남",
        place: "천안삼거리공원",
        distances: ["100km", "60km"],
        status: "대회 취소",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 서울레이스",
        date: "2026-10-11",
        region: "서울",
        place: "서울광장",
        distances: ["하프", "10km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 MBN 나주마라톤",
        date: "2026-10-11",
        region: "전남",
        place: "나주종합스포츠파크",
        distances: ["풀", "하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 진안고원 Forestival Run Heal",
        date: "2026-10-11",
        region: "전북",
        place: "국립진안고원산림치유원",
        distances: ["16km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://rocknrun.kr/Jinangowon_Forestival"
    },

    {
        name: "2026 당진 황금들판 마라톤",
        date: "2026-10-11",
        region: "충남",
        place: "당진 삽교 관광단지 헬기장",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 인천로봇랜드 퓨처런",
        date: "2026-10-11",
        region: "인천",
        place: "인천로봇랜드 로봇타워",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://futurerun.co.kr/"
    },

    {
        name: "2026 Univ Run",
        date: "2026-10-11",
        region: "서울",
        place: "구일역 안양천 축구장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제22회 금천건강달리기",
        date: "2026-10-11",
        region: "서울",
        place: "안양천 다목적광장",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제10회 가을철 산불예방 마라톤",
        date: "2026-10-11",
        region: "서울",
        place: "광평교운동장",
        distances: ["풀", "31km", "하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 평택항마라톤",
        date: "2026-10-11",
        region: "경기",
        place: "평택항 엠에스로지스틱",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.ptportmarathon.com/"
    },

    {
        name: "2026 문경오미자 하프마라톤",
        date: "2026-10-11",
        region: "경북",
        place: "문경시민운동장",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 핑크런",
        date: "2026-10-11",
        region: "서울",
        place: "상암월드컵공원 평화광장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.pinkrun.kr/"
    },

    {
        name: "2026 울산마라톤",
        date: "2026-10-11",
        region: "울산",
        place: "울산종합운동장",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://run.uaf.co.kr/"
    },

    {
        name: "2026 천안 이봉주마라톤",
        date: "2026-10-11",
        region: "충남",
        place: "천안종합운동장",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제6회 영등포 육상대회",
        date: "2026-10-11",
        region: "서울",
        place: "신정교하부육상트랙구장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 함께런 러닝 페스티벌",
        date: "2026-10-11",
        region: "경기",
        place: "미사경정공원 관리동 광장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 대전 Dtrail",
        date: "2026-10-17",
        region: "대전",
        place: "보문산야외음악당",
        distances: ["50km", "25km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://dtrail.co.kr/"
    },

    {
        name: "2026 경주마라톤",
        date: "2026-10-17",
        region: "경북",
        place: "경주시민운동장",
        distances: ["풀", "하프", "10km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.gyeongjumarathon.com/"
    },

    {
        name: "제24회 청원생명쌀 대청호마라톤",
        date: "2026-10-17",
        region: "충북",
        place: "문의체육공원",
        distances: ["풀", "하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 정선 동강마라톤",
        date: "2026-10-17",
        region: "강원",
        place: "정선생태체험학습장",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제7회 태안 전국어울림마라톤",
        date: "2026-10-17",
        region: "충남",
        place: "태안군 안면읍 꽃지해수욕장 일원",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://xn--289av7rnxbd06bmrc.com/"
    },

    

    {
        name: "제2회 1019 평화마라톤",
        date: "2026-10-17",
        region: "전남",
        place: "여수진남경기장 주경기장",
        distances: ["5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 광주 남구 마라톤",
        date: "2026-10-17",
        region: "광주",
        place: "광주공원 앞 광주천변 광주교",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 의정부 워터마라톤",
        date: "2026-10-17",
        region: "경기",
        place: "동막교인라인광장",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제14회 마이크로 패밀리 킥앤런",
        date: "2026-10-17",
        region: "경기",
        place: "하남 미사 경정공원",
        distances: [],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 Uplus 119 Memorial Run",
        date: "2026-10-17",
        region: "세종",
        place: "세종중앙공원 도시축제마당",
        distances: ["11.9km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://119memorial.com/"
    },

    {
        name: "2026 억새노을마라톤",
        date: "2026-10-17",
        region: "광주",
        place: "상무시민공원",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    

    {
        name: "2026 아시아오픈마라톤",
        date: "2026-10-18",
        region: "서울",
        place: "서울 광화문광장",
        distances: ["하프", "10km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "Greenstep 2026",
        date: "2026-10-18",
        region: "전남",
        place: "전라남도 해남군 솔라시도",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제22회 북구사랑마라톤대회",
        date: "2026-10-18",
        region: "대구",
        place: "금호강 산격야영장",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제25회 대청호마라톤",
        date: "2026-10-18",
        region: "대전",
        place: "대청공원",
        distances: ["하프", "10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://dcmarathon.or.kr/"
    },

    {
        name: "제23회 여주 세종대왕마라톤",
        date: "2026-10-18",
        region: "경기",
        place: "여주 현암지구공원",
        distances: ["하프", "10km", "4km", "1km 휠체어"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제19회 중랑 장미마라톤",
        date: "2026-10-18",
        region: "서울",
        place: "중화체육공원",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "제28회 창원시민건강달리기",
        date: "2026-10-18",
        region: "경남",
        place: "창원스포츠파크 보조경기장",
        distances: ["6km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 담양 트레일런 데이브레이크",
        date: "2026-10-24",
        region: "전남",
        place: "담양 추성경기장",
        distances: ["29K+", "9K+"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 서울 펫림픽런 With Go",
        date: "2026-10-24",
        region: "서울",
        place: "서울 월드컵경기장 평화광장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 SBS 런 뮤직 페스티벌 안동",
        date: "2026-10-24",
        region: "경북",
        place: "안동 탈춤공원",
        distances: ["하프", "10km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 제3회 감성런",
        date: "2026-10-24",
        region: "서울",
        place: "신정교하부육상트랙구장",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 익산 기부런",
        date: "2026-10-24",
        region: "전북",
        place: "익산시종합운동장",
        distances: ["10km", "6.4km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://iksangiverun.or.kr/"
    },

    {
        name: "2026 하남 가을 패밀리 마라톤",
        date: "2026-10-24",
        region: "경기",
        place: "하남 미사 경정공원",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 경기도 옛길 함께달리기",
        date: "2026-10-24",
        region: "경기",
        place: "도농체육공원",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 세종런 26",
        date: "2026-10-24",
        region: "세종",
        place: "세종중앙공원 도시축제마당",
        distances: ["10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://sejongrun26.com/"
    },

    {
        name: "2026 뉴시스 씨 마라톤",
        date: "2026-10-24",
        region: "인천",
        place: "영종도 씨사이드파크 하늘구름광장",
        distances: ["하프", "10km", "5km"],
        status: "접수 중",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 무서런",
        date: "2026-10-24",
        region: "충북",
        place: "무심천체육공원",
        distances: ["10km", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 불닭 버닝 트레일런",
        date: "2026-10-24",
        region: "강원",
        place: "대관령 삼양라운드힐",
        distances: ["23K", "13K", "5km"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: ""
    },

    {
        name: "2026 광명평화마라톤",
        date: "2026-10-25",
        region: "경기",
        place: "광명역",
        distances: ["하프", "10km", "5km", "5km 커플런"],
        status: "접수 마감",
        price: "공식 홈페이지 확인",
        period: "",
        url: "https://www.gmrun.kr/"
    },
      {
    name: "2026 생명사랑 밤길걷기 인천",
    date: "2026-09-01",
    region: "인천",
    place: "인천 전역 / 원하는 장소",
    distances: ["걷기"],
    status: "접수 중",
    price: "공식 홈페이지 확인",
    period: "2026-09-01 ~ 2026-09-30",
    url: "https://incheon.onesteponelife.com"
  },
  {
    name: "2026 인제베어와 함께하는 테디서킷런",
    date: "2026-10-05",
    region: "강원",
    place: "인제 스피디움",
    distances: ["5km", "10km"],
    status: "접수 중",
    price: "65,000원",
    period: "현장 신청",
    url: "https://teddymarathon.com/"
  },
  {
    name: "굿네이버스 레이스 with 띵크어스 - 광주 레이스",
    date: "2026-10-17",
    region: "광주",
    place: "황룡친수공원 일원",
    distances: ["5km"],
    status: "접수 중",
    price: "30,000원",
    period: "2026-09-07 ~ 2026-09-30",
    url: "https://gnrace.kr/"
  },
  {
    name: "2026 경청마라톤",
    date: "2026-10-17",
    region: "서울",
    place: "신정교 하부 영롱이 억새구장",
    distances: ["하프", "10km", "5km"],
    status: "접수 중",
    price: "공식 홈페이지 확인",
    period: "2026-07-07 ~ 2026-10-07",
    url: ""
  },
  {
    name: "2026 옥스팜 트레일워커 서울",
    date: "2026-10-17",
    region: "서울",
    place: "한국프레스센터 광장 ~ 세종로공원",
    distances: ["20km"],
    status: "접수 중",
    price: "150,000원/팀",
    period: "2026-08-03 ~ 2026-09-28",
    url: "https://www.oxfamtrailwalker.or.kr/ko/seoul-20k"
  },
  {
    name: "한탄강 울트라 레이크 트레일",
    date: "2026-10-18",
    region: "경기",
    place: "한탄강 세계지질공원 내 다목적광장",
    distances: ["52K", "25K", "13K"],
    status: "접수 중",
    price: "52K 160,000원 / 25K 100,000원 / 13K 60,000원",
    period: "2026-10-17 ~ 2026-10-18",
    url: "https://ultralaketrail.com/"
  },
  {
    name: "Freedom Walk & Run 2026",
    date: "2026-10-24",
    region: "서울",
    place: "반포한강공원 세빛섬 주변 한강변 보행로",
    distances: ["6km"],
    status: "접수 중",
    price: "50,000원",
    period: "2026-09-02 ~ 2026-10-20",
    url: "https://freedomwalk.run/2026g"
  },
  {
    name: "2026 오티즘 레이스",
    date: "2026-10-31",
    region: "서울",
    place: "상암 평화의공원 평화광장",
    distances: ["하프", "10km", "4.2km"],
    status: "접수 마감",
    price: "40,000원",
    period: "2026-07-08 ~ 2026-09-09",
    url: "https://www.autismrace.com/"
  },
  {
    name: "2026 JUST RUN10 포천",
    date: "2026-10-31",
    region: "경기",
    place: "포천시종합운동장",
    distances: ["10km", "5km"],
    status: "접수 마감",
    price: "공식 홈페이지 확인",
    period: "2026-08-11 ~ 2026-09-11",
    url: ""
  },
  {
    name: "제7회 중소기업인 氣살리기 마라톤대회",
    date: "2026-10-31",
    region: "서울",
    place: "안양천 축구장 (1호선 구일역 하부)",
    distances: ["5km", "10km", "하프"],
    status: "접수 중",
    price: "5km 44,000원 / 10km·하프 55,000원",
    period: "2026-08-07 ~ 2026-10-18",
    url: "https://kasmirun.kr/"
  },
  {
    name: "2026 김천 전국마라톤 대회",
    date: "2026-11-01",
    region: "경북",
    place: "김천종합스포츠타운 보조경기장",
    distances: ["하프", "10km", "5km"],
    status: "접수 마감",
    price: "공식 홈페이지 확인",
    period: "2026-09-18까지",
    url: "https://gcmarathon.co.kr/"
  },
  {
    name: "2026 에그런",
    date: "2026-11-07",
    region: "세종",
    place: "국립세종수목원 축제마당",
    distances: ["5km"],
    status: "접수 중",
    price: "정기후원 5,000원",
    period: "2026-09-07 ~ 2026-10-07",
    url: "https://eggrun2026.imweb.me/"
  },
  {
    name: "제22회 국방일보 전우마라톤대회",
    date: "2026-11-07",
    region: "경기",
    place: "임진각 평화누리공원",
    distances: ["10km", "5km"],
    status: "접수 중",
    price: "20,000원",
    period: "2026-09-07 ~ 2026-10-08",
    url: "https://www.dema.mil.kr/marathon/intro/mainPoint.do"
  },
  {
    name: "런텐 13차 레이스 JUST RUN10 대전",
    date: "2026-11-07",
    region: "대전",
    place: "엑스포다리 인근",
    distances: ["10km", "5km"],
    status: "접수 중",
    price: "공식 홈페이지 확인",
    period: "",
    url: ""
  },
  {
    name: "레디, 셋, 공익런!",
    date: "2026-11-07",
    region: "충북",
    place: "청주 무심천체육공원 롤러스케이트장",
    distances: ["10km", "5km"],
    status: "접수 중",
    price: "25,000원",
    period: "2026-09-20 확인 / ~2026-10-06",
    url: "https://app.notion.com/p/2026-3c97e45d0696800ca05dde948858ed26"
  },
  {
    name: "2026 라운드랩 RUN",
    date: "2026-11-08",
    region: "강원",
    place: "춘천 송암스포츠타운 종합경기장",
    distances: ["하프", "10km", "5km"],
    status: "접수 중",
    price: "공식 홈페이지 확인",
    period: "2026-08-07 ~ 선착순 마감",
    url: "https://www.kw-marathon.com/"
  },
  {
    name: "제3회 자유민주 마라톤대회",
    date: "2026-11-08",
    region: "서울",
    place: "서울광장",
    distances: ["10km", "6.10km"],
    status: "접수 중",
    price: "10km 40,000원 / 6.10km 35,000원",
    period: "2026-09-04 ~ 선착순 마감",
    url: "https://610run.kr/"
  },
  {
    name: "2026 강남하프마라톤",
    date: "2026-11-08",
    region: "서울",
    place: "대치유수지 체육공원",
    distances: ["하프", "10km"],
    status: "접수 중",
    price: "하프 65,000원 / 10km 60,000원",
    period: "2026-09-14 ~ 2026-10-23",
    url: "https://comport11.oopy.io/"
  },
    {
    name: "2026 통큰런",
    date: "2026-11-14",
    region: "서울",
    place: "상암 월드컵공원 평화광장",
    distances: ["10km", "5km", "3km"],
    status: "접수 중",
    price: "30,000원",
    period: "2026-09-14 ~ 2026-10-23",
    url: "https://tongkeun-run.com/"
},

{
    name: "2026 영종 하늘길 마라톤",
    date: "2026-11-14",
    region: "인천",
    place: "영종 씨사이드파크 일대",
    distances: ["하프", "10km", "5km"],
    status: "접수 중",
    price: "하프 55,000원 / 10km 55,000원 / 5km 45,000원",
    period: "2026-08-24 ~ 2026-10-08",
    url: "https://www.leaveit.co.kr/match/apply/2026-yeongjong-airway"
},

{
    name: "스타콜 × 롤링스톤 레트로 런 IN 증평",
    date: "2026-11-14",
    region: "충북",
    place: "증평군 보강천 시민체육공원",
    distances: ["10km", "5km"],
    status: "접수 중",
    price: "10km 79,000원 / 5km 69,000원",
    period: "2026-09-15 ~ 2026-10-25",
    url: "https://n-runplus.co.kr/Jeungpyeong"
},

{
    name: "제9회 서울평화마라톤",
    date: "2026-11-15",
    region: "서울",
    place: "신정교 하부 육상트랙구장",
    distances: ["10km", "5km"],
    status: "접수 중",
    price: "10km 50,000원 / 5km 35,000원",
    period: "2026-07-08 ~ 2026-10-16",
    url: ""
},

{
    name: "RUN & ESPRESSO 10K SEOUL",
    date: "2026-11-15",
    region: "서울",
    place: "서울 상암 월드컵공원 평화광장",
    distances: ["10km"],
    status: "접수 중",
    price: "공식 홈페이지 확인",
    period: "",
    url: "https://www.fila.co.kr/pages/fila-run-espresso-10k-seoul"
},

{
    name: "서울 라이프마라톤",
    date: "2026-11-21",
    region: "서울",
    place: "상암 평화의공원 평화광장",
    distances: ["하프", "10km", "5km"],
    status: "접수 중",
    price: "하프 59,000원 / 10km 55,000원 / 5km 49,000원",
    period: "",
    url: "https://seoul.lifemarathon.co.kr/overview"
},

{
    name: "JUST RUN10 대구",
    date: "2026-11-21",
    region: "대구",
    place: "금호강 러닝성지·화랑교 아래",
    distances: ["10km", "5km"],
    status: "접수 중",
    price: "공식 홈페이지 확인",
    period: "2026-09-11 ~ 2026-10-20",
    url: "https://runten.co.kr/"
},

{
    name: "2026 인천마라톤",
    date: "2026-11-22",
    region: "인천",
    place: "인천아시아드 주경기장 / 운서역 일원",
    distances: ["풀", "10km", "5km"],
    status: "접수 마감",
    price: "풀 80,000원 / 10km 50,000원 / 5km 30,000원",
    period: "2026-09-07 ~ 2026-09-21",
    url: "https://www.incheonmarathon.or.kr/"
},

{
    name: "2026 평화의숲 마라톤",
    date: "2026-11-22",
    region: "서울",
    place: "상암 월드컵공원 평화광장",
    distances: ["10km", "5km"],
    status: "접수 중",
    price: "공식 홈페이지 확인",
    period: "",
    url: "https://peace-run.kr/"
},

{
    name: "제14회 안중근 평화 마라톤",
    date: "2026-11-29",
    region: "서울",
    place: "여의도공원 문화의마당",
    distances: ["하프", "10km", "5km"],
    status: "접수 중",
    price: "공식 홈페이지 확인",
    period: "~ 2026-11-22",
    url: "https://www.ajgrun.kr/"
},

{
    name: "2026 하남 미사 호수 에너지런",
    date: "2026-12-05",
    region: "경기",
    place: "하남 미사 경정공원",
    distances: ["10km", "5km"],
    status: "접수 중",
    price: "50,000원",
    period: "~ 2026-11-26",
    url: "https://energyrun.kr/"
},

{
    name: "제23회 한강시민마라톤",
    date: "2026-12-05",
    region: "서울",
    place: "여의도 물빛무대 앞 광장",
    distances: ["하프", "10km", "5km"],
    status: "접수 중",
    price: "50,000원",
    period: "2026-08-01 ~ 2026-10-28",
    url: "https://www.hangangmarathon.kr/"
},

{
    name: "2026 황영조 로드레이스",
    date: "2026-12-06",
    region: "인천",
    place: "경인아라뱃길 여객터미널 아라타워 일원",
    distances: ["하프", "10km"],
    status: "접수 중",
    price: "50,000원",
    period: "2026-08-14 ~ 2026-10-30",
    url: "https://roadracing.co.kr/"
},

{
    name: "제3회 긍정의힘 마라톤",
    date: "2026-12-12",
    region: "서울",
    place: "신정교 하부 영롱이 억새구장",
    distances: ["하프", "10km", "5km"],
    status: "접수 중",
    price: "50,000원",
    period: "2026-08-30 ~ 2026-12-06",
    url: "https://www.powerrun.kr/"
},

{
    name: "제5회 밤섬 마라톤",
    date: "2026-12-13",
    region: "서울",
    place: "상암 월드컵공원 평화광장",
    distances: ["하프", "10km", "5km"],
    status: "접수 중",
    price: "60,000원",
    period: "",
    url: "https://www.bamseom.com/"
},

{
    name: "제2회 청춘 릴레이 마라톤",
    date: "2026-12-19",
    region: "서울",
    place: "상암 월드컵공원 평화광장",
    distances: ["하프", "10km", "5km"],
    status: "접수 중",
    price: "50,000원",
    period: "2026-08-12 ~ 2026-12-09",
    url: "https://youngrun.kr/"
},

{
    name: "2026 코리아 시즌 레이스",
    date: "2026-12-26",
    region: "서울",
    place: "상암 월드컵공원 평화광장",
    distances: ["하프", "10km", "5km"],
    status: "접수 중",
    price: "50,000원",
    period: "2026-08-25 ~ 2026-12-16",
    url: "https://k-run.kr/"
},
];

console.log("여기런 대회 수:", races.length);

// 브라우저에서는 전역 변수로 사용
// GitHub Actions / Node.js에서는 require()로 사용
if (typeof module !== "undefined" && module.exports) {
    module.exports = races;
}
